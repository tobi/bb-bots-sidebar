...
export function useConversationReorder(threads: PluginSidebarThread[], onReorder: (source: string, target: string, position: "before" | "after") => void, onNest: (source: string, target: string) => void) {
  ...
  const target = drop.drag?.botId ? JSON.parse(drop.drag.botId) as { id: string; position: "before" | "after" | "inside" } : null;
  return { ...drop, target };
}
