...
describe("Bots sidebar", () => {
  it("does not count independent topics as children of the main conversation", async () => {
    const { slot } = await mount();
    const row = slot.getByText("Test bot").closest<HTMLElement>(".project-row")!;
    expect(row.querySelector(".conversation-disclosure")).toBeNull();
    expect(slot.container.querySelectorAll("[data-sidebar-thread-id]")).toHaveLength(5);
    expect(slot.getByRole("group", { name: "Top-level conversations for Test bot" })).toBeTruthy();
  ...
  it("folds older inactive roots with only the requested archive action", async () => {
    const { slot } = await mount();
    expect(slot.container.querySelectorAll("[data-sidebar-thread-id]")).toHaveLength(5);
    expect(slot.container.querySelectorAll(".thread-row button:not(.conversation-archive), .project-row-actions")).toHaveLength(0);
    expect(slot.container.querySelectorAll(".thread-row .conversation-archive")).toHaveLength(5);
    fireEvent.click(slot.getByRole("button", { name: "5 Other" }));
    expect(slot.container.querySelectorAll("[data-sidebar-thread-id]")).toHaveLength(10);
    expect(slot.inspection.sidebarActionCalls).toEqual([]);
  });
  it.each([
  ...
  it("retains Make main in the conversation menu", async () => {
    const { slot } = await mount();
    fireEvent.contextMenu(slot.getByText("Conversation 0"));
    fireEvent.click(await slot.findByRole("menuitem", { name: "Make main conversation" }));
    await waitFor(() => expect(slot.inspection.rpcCalls).toContainEqual({ method: "main_set", input: { botId: bot.id, threadId: "0" } }));
  });
