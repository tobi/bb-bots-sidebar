...
export function ConversationArchiveButton({ thread, actions, onArchive }: { thread: PluginSidebarThread; actions: Pick<PluginSidebarThreadActions, "archive">; onArchive?: (threadId: string) => void }) {
  ...
  return <Tooltip>
    <TooltipTrigger asChild>
      <Button type="button" variant="ghost" size="icon" className="conversation-archive h-6 w-6 shrink-0 p-0 text-muted-foreground" aria-label={`Archive ${title}`} data-archive-thread-id={thread.id}
        ...
        onClick={(event) => { event.preventDefault(); event.stopPropagation(); onArchive?.(thread.id); actions.archive(thread.id); }}>
